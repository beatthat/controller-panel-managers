...
//						newPanel.Unbind(); // if p is already the active panel, allow transitions to handle the unbind
					...
					if(newPanelHasTrans != null) {
						...
						if(m_suspend) {
							t.AddAction(() => { if(newPanelGO != null) { newPanelGO.SetBool<Suspend> (false); } });
						}
					}
					else {
						ActivatePanel(req);//req.Go();
...
//					UpdateDisplay();
		...
		public void ActivatePanel(ChangePanel p)
		{
			if(p.panelGO == null) {
				return;
			}

			var ctl = p.panel as IController;
			if(ctl != null) {
				ActivateController(ctl, ControllerPanelOptions.OPT_MODEL.Get(p.options));
				return;
			}

			p.panelGO.SetActive(true);

			if((ctl = p.panelGO.GetComponent<IController>()) != null) {
				ActivateController(ctl, ControllerPanelOptions.OPT_MODEL.Get(p.options));
				return;
			}
		}
